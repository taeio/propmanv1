...
export default async function handler(
  ...
  try {
    ...
    if (req.method === "GET") {
      return compose(
        withRateLimit({ windowMs: 60000, maxRequests: 100, routePattern: '/api/data/projects/[id]' }),
        requireAuth,
    ...
    if (req.method === "PUT") {
      return compose(
        withRateLimit({ windowMs: 60000, maxRequests: 30, routePattern: '/api/data/projects/[id]' }),
        requireCsrf,
        requireAuth,
        requireRole("property_manager"),
